...
class FMPClient:
    ...
    def get_earnings_calendar(self, from_date: str, to_date: str) -> Optional[list]:
        ...
        Returns:
            List of earnings announcements or None on failure. Each entry
            carries at least ``symbol`` and ``date``. (The /stable calendar
            no longer reports an intraday ``time`` field; callers treat a
    ...
    def _get_company_profiles_v3(self, symbols: list[str]) -> dict[str, dict]:
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            cache_key = f"profiles_v3_{symbols_str}"
            data = self.cache.get(cache_key)
            if data is None:
                data = self._rate_limited_get(f"{self.BASE_URL}/profile/{symbols_str}")
                if data:
                    self.cache[cache_key] = data
            if data:
                for profile in data:
                    if isinstance(profile, dict):
                        profiles[profile.get("symbol")] = self._v3_compat_profile(profile)
        return profiles
    ...
    def get_company_profiles(self, symbols: list[str]) -> dict[str, dict]:
        ...
        — far cheaper than one request per symbol. Falls back to the v3 batched
        profile endpoint for legacy keys where profile-bulk is unavailable.
